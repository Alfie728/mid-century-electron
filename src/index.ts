...
// WORKAROUND for macOS 26 (Tahoe) compatibility
// Electron 39's sandbox crashes during native initialization on macOS 26.
// This won't help if the crash happens before JavaScript loads, but keeps
// the code in place for when Electron eventually fixes the native crash.
// Users on macOS 26 should run the app with --no-sandbox flag:
//   /path/to/electron.app/Contents/MacOS/electron --no-sandbox
...
function getIohookMacos() {
  if (!_iohookMacos) {
    // Handle both ESM default export and CommonJS module.exports
    const mod = require("iohook-macos");
    _iohookMacos = mod.default || mod;
  }
  return _iohookMacos;
}
...
ipcMain.handle("checkAccessibilityPermission", () => {
  if (process.platform === "darwin") {
    try {
      const iohookMacos = getIohookMacos();
